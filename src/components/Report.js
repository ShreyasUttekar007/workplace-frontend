...
const Report = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const momDataResponse = await api.get(
          `${API_URL}/report/get-report/${userId}`,
          {
            headers: {
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByPC = await api.get(
            `${API_URL}/report/get-report-by-pc/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByConstituency = await api.get(
            `${API_URL}/report/get-report-by-constituency/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByZone = await api.get(
            `${API_URL}/report/get-report-by-zone/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleDeleteMom = async (momId) => {
    try {
      const token = await localforage.getItem("token");
      await api.delete(`${API_URL}/report/delete-report/${momId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  useEffect(() => {
    const fetchDocumentCount = async () => {
      try {
        ...
        const response = await api.get(`${API_URL}/report/count-documents`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    const fetchForm20DocumentCount = async () => {
      try {
        ...
        const response = await api.get(
          `${API_URL}/report/count-form20-documents`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
              ...
              >
                <div className="total-count" style={{ margin: "5px" }}>
                  ...
                  {location === "Andhra Pradesh" ? null : (
                    <>
                      <h3>Total Form-20: </h3>
                      <h6>{filteredForm20DocumentCount}</h6>
                    </>
                  )}
                  {/* Updated line */}
                </div>
              ...
              <table className="mom-table">
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: "center" }}>AC Level Reports</th>
                    {location === "Andhra Pradesh" ? (
                      <th style={{ textAlign: "center" }}>PC MoM</th>
                    ) : (
                    ...
                    <tr key={mom.id}>
                      ...
                      <td className="td-extra">
                        ...
                        >
                          ...
                          {location ===
                          "Maharashtra" ? null : mom.acVsCubReports ? (
                            <a
                              href={mom.acVsCubReports}
                          ...
                          {location ===
                          "Maharashtra" ? null : mom.apAcReports ? (
                            <a
                              href={mom.apAcReports}
                              download={`Document_of_${mom.leaderName}.pdf`}
                              className="download-link"
                              target="_blank"
                              style={{ color: "#008cff" }}
