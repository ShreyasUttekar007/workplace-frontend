...
const AddProbableJoinees = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/probable-joinee/joinee`, payload, {
        headers: {
  ...
  const handleFileUpload = async (e, fieldName) => {
    ...
    try {
      const uploadedUrl = await uploadFileToS3(file, "joinee photos"); // 👈 uploading into 'photos' folder in your S3
      if (uploadedUrl) {
        setFormData((prevData) => ({
          ...prevData,
          [fieldName]: uploadedUrl, // fieldName will be 'photo'
        }));
      }
    } catch (error) {
...
export default AddProbableJoinees;
