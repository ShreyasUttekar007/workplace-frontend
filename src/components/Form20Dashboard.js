...
const Form20Dashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const momDataResponse = await api.get(
          `${API_URL}/form20/get-report/${userId}`,
          {
            headers: {
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByPC = await api.get(
            `${API_URL}/form20/get-report-by-pc/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByConstituency = await api.get(
            `${API_URL}/form20/get-report-by-constituency/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
      ...
      try {
        ...
        if (value === initialDropdownValue) {
          ...
          const responseMomByZone = await api.get(
            `${API_URL}/form20/get-report-by-zone/${value}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const handleDeleteMom = async (momId) => {
    try {
      const token = await localforage.getItem("token");
      await api.delete(
        `${API_URL}/form20/delete-report/${momId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
