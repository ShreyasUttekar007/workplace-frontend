...
const UpdateDriverData = () => {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/cab/vendors`, {
  ...
  useEffect(() => {
    const fetchCabRequest = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/cab/cab-requests-emails`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = await localforage.getItem("token");
      await axios.put(`${API_URL}/cab/update-cab-data/${momId}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
