        ...
        {validPeoplePhotos.length > 0 && (
          <div className="mom-new-section page-break">
            <h3 className="heading-mom-texts">
              Images of those mentioned in minutes:
            </h3>
            <table className="mentioned-people-table">
              <tbody>
                <tr>
                  {validPeoplePhotos.map((photo, index) => (
                    <td key={index} className="mom-image-item">
                      <img
                        src={photo}
                        alt={momData.peopleName[index] || "Unknown Person"}
                        onError={(e) => (e.target.style.display = "none")} // Hides broken images
                      />
                      {momData.peopleName[index] && (
                        <p className="extra-image-text">
                          {momData.peopleName[index]}
                        </p>
                      )}
                      {momData.peopleDesignation[index] &&
                        momData.peopleParty[index] && (
                          <p>
                            {momData.peopleDesignation[index]},{" "}
                            {momData.peopleParty[index]}
                          </p>
